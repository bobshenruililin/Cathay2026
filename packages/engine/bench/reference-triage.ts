import {
  ...
  isValidItinerary,
  slackMinutes,
} from "../src/feasibility";
import { UM_ESCORT_BUFFER_MINUTES, WHEELCHAIR_TRANSIT_BUFFER_MINUTES } from "../src/mct";
import { isUnaccompaniedMinor, needsWheelchair, partySizeOf } from "../src/passenger";
import type { Connection, Flight, TriageResult } from "../src/types";
...
/** Independent of `src/triage.ts`. Options from the sort-based reference, not the impl. */
export function referenceTriage(connection: Connection, pool: readonly Flight[] = []): TriageResult {
  const base = {
  ...
  const options = atRisk ? referenceOptions(connection, pool) : [];
  const reasoning = [
